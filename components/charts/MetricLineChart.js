...
function getSimilarColor(inputColor,i,dataLen) {
  ...
  const newR = Math.abs((r + factor * 2 * i) % 256);
  const newG = Math.abs((g + factor * 0.4 * i) % 256);
  const newB = Math.abs((b + factor * 0.25 * i) % 256);
  ...
  async function getData(path) {
      ...
      const data = {
          ...
          datasets: cols.map((val,i) => {
              let color = getSimilarColor("#A88A53", i, cols.length)
              return {
                  label: cols[i],
                  data: groupData[i],
                  borderColor: color,
