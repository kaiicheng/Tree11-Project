...
class SocrataClient:
    def __init__(self, token=None, timeout=30, retries=4, max_dataset_seconds=1200,
    ...
    def rows(self, dataset_id, order="updateddate,globalid", limit=None, page_size=None,
        ...
        self._event("dataset_started", source=name, dataset_id=dataset_id,
                    pagination_strategy="offset", initial_page_size=current_size,
        ...
        while True:
            ...
            for attempt in range(1, self.retries + 1):
                ...
                if remaining <= 1:
                    self._fail_deadline(dataset_id, total)
                params = {"$limit": requested, "$offset": offset, "$order": order}
                if where:
                    params["$where"] = where
                ...
                try:
                    response = self.session.get(
                        ...
                        params=params, headers=self.headers, timeout=min(self.timeout, max(1, remaining)),
                    )
                    ...
                    if response.ok:
                        ...
                        failure = ("http_429", "HTTP 429", False)
                ...
                delay = float(response.headers.get("Retry-After", min(2 ** (attempt - 1), 20))) if reason == "http_429" and response is not None else min(2 ** (attempt - 1), 20)
        ...
        self.last_stats = {
            "pagination_strategy": "offset", "initial_page_size": config.initial,
            "minimum_page_size": config.minimum, "maximum_page_size": config.maximum,
